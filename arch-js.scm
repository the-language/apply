...
(define ($$define/k local-state state f x k) (k (hash-set local-state '_ (cons f (hash-ref local-state '_))) state (list (**set! f x))))
(define $void "VOIDz")
(define $null "NULLz")
...
(define ($$top local-state state xs) (**top (append (map **define-undefined (hash-ref local-state '_)) xs)))
(define $arch 'js)
